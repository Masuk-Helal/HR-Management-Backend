...
def authenticate_user(username, password, db):
    ...
    if user is None:
        return False
    if bcrypt_context.verify(truncate_password(password), user.hash_password):
        return user
    return False


def create_access_token(username: str, user_id: int, role: str, email: str, firstname: str, lastname: str, expires_delta: timedelta):
    encode = {'sub': username, 'id': user_id, 'role': role, 'username': username, 'email': email, 'firstname': firstname, 'lastname': lastname}
...
def get_current_user(token: Annotated[str, Depends(OAuth2_bearer)]):
    try:
        ...
        return {'username': username, 'id': user_id, 'role': role, 'email': email, 'firstname': firstname, 'lastname': lastname}
    except:
        raise HTTPException(status_code=404, detail='User not found')
...
def create_users(db : db_dependency, new_user : CreateUsers):
    user_model = Users(
        ...
        hash_password = bcrypt_context.hash(truncate_password(new_user.password)),
        is_active = True,
        role = new_user.role
    )
...
def login_user(db : db_dependency, form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    token = create_access_token(user.username, user.id, user.role, user.email, user.firstname, user.lastname, timedelta(minutes=30))
